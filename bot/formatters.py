...
def fmt_arb_alert(
    ...
    return (
        ...
        f"<b>Arb:</b> <code>{opp.get('net_pct_top', opp['net_profit_pct']):.2f}%</code>  ·  "
        f"<b>max profit</b> <code>${opp.get('max_profit_usd', opp['net_profit_usd']):.2f}</code> "
        f"on <code>${opp.get('max_wager_usd', opp['notional_usd']):.0f}</code>\n\n"
        f"{calc_line}"
...
def fmt_arb_list(opps: list[dict], min_pct: float = 3.0, min_usd: float = 5.0, site_url: str = "") -> str:
    ...
    for i, o in enumerate(opps[:10], 1):
        ...
        lines.append(
            ...
            f"on <code>${o.get('max_wager_usd', 0):.0f}</code>\n"
            f"{calc}"
            f"   <a href=\"{poly_url}\">Polymarket</a>  |  <a href=\"{pf_url}\">Predict.fun</a>"
        )
    return "\n".join(lines)
